...
4. 涉及删除/修改类操作（k8sdel、k8sedit、删 pod、改线上资源）**以及会写数据的脚本**（数据修复/迁移/删除），必须先 AskUserQuestion 确认（提问与选项一律用中文）
...
```bash
# <app> 为 config envs.test.k8s.apps 里的键
...
ssh -t -p <port> "$JMS" "kubectl exec -it -n <ns> $POD -c <container> -- bin/rails c"   # rails console（Rails 应用；非 Rails 无此命令，直接进 bash 跑脚本）
